          ...
          gpointer        user_data)
{
  ...
  gtk_grid_attach(GTK_GRID(grid), database_label, 0, 2, 1, 1);

  database_entry = gtk_entry_new();
  gtk_grid_attach(GTK_GRID(grid), database_entry, 1, 2, 1, 1);

  GtkWidget *ip_label;
  ip_label = gtk_label_new("IP Address:");
  gtk_grid_attach(GTK_GRID(grid), ip_label, 0, 3, 1, 1);

  ip_entry = gtk_entry_new();
  gtk_grid_attach(GTK_GRID(grid), ip_entry, 1, 3, 1, 1);
